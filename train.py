...
def train():
    ...
    depth_ckpt = '/root/autodl-tmp/unsup_experiment/checkpoints/crop_unsup_resnet18_photo1_pso1_0211/train-md4allDDa-robotcar-epoch=14.ckpt'
    texture_ckpt = '/root/autodl-tmp/lidar_experiment/checkpoints/texturetrain_robotcar_sparse_sup_unidepth_l_distill_night_resnet18_sup075_pso025_multi_scale_0126/train-md4allDDa-robotcar-epoch=06.ckpt'
    full_state_dict = {}
    if os.path.exists(depth_ckpt):
        checkpoint = torch.load(depth_ckpt)
        for key, value in checkpoint['state_dict'].items():
            if not 'texture' in key:  # 深度网络相关权重
                full_state_dict[key] = value
        
    if os.path.exists(texture_ckpt):
        checkpoint = torch.load(texture_ckpt)
        for key, value in checkpoint['state_dict'].items():
            if 'texture' in key or 'injection' in key:  # 纹理网络相关权重
                full_state_dict[key] = value

    # 加载完整的权重
    print(f"Total parameters in full state dict: {sum(p.numel() for p in full_state_dict.values())}")
    model.load_state_dict(full_state_dict, strict=True)

    for name, param in model.named_parameters():
        param.requires_grad = True
        print(f"Training parameter: {name}")
    # model.texture_model.load_state_dict(state_dict={key.replace("texture_model.", ""): weight for key, weight in torch.load(ckpt_path)['state_dict'].items() if 'texture_model' in key})
