...
_C.TRAINING = CN()
_C.TRAINING.EPOCHS = 32  # Number of epochs to train
_C.TRAINING.BATCH_SIZE = 8  # Batch size for training
...
_C.VALIDATION = CN()
_C.VALIDATION.BATCH_SIZE = 8  # Batch size for validation

_C.SAVE = CN()
_C.SAVE.CHECKPOINT_PATH = 'log/checkpoints'  # Folder within md4all to store checkpoints
_C.SAVE.LOG_DIR = '/root/autodl-tmp/unsup_experiment/log/training'  # Directory to store tensorboard logs
...
_C.SAVE.MONITOR = 'metrics_night/everything/abs_rel_pp_gt'  # Metric to monitor
_C.SAVE.TOP_K = 4  # Number of top models (w.r.t. monitoring metric) to save
...
_C.LOSS.PSOSUPERVISED = CN()
_C.LOSS.PSOSUPERVISED.WEIGHT = 0.0  # Supervised loss weight
_C.LOSS.PSOSUPERVISED.METHOD = 'sasi'  # Supervised loss type
_C.LOSS.TEXTURE = CN()
_C.LOSS.TEXTURE.WEIGHT = 0.0

_C.OPTIMIZER = CN()
_C.OPTIMIZER.DEPTH_LR = 2e-5  # Learning rate for depth network
# _C.OPTIMIZER.TEXTURE_LR = 2e-6  # Learning rate for pose network
_C.OPTIMIZER.POSE_LR = 2e-5  # Learning rate for pose network
...
_C.EVALUATION = CN()
_C.EVALUATION.BATCH_SIZE = 8  # Evaluation batch size
