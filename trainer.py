...
from models.depth_net_texture import TextureAwareDepthNet
...
class Md4All(pl.LightningModule):
    ...
    def __init__(self, cfg, is_train):
        ...
        # self.texture_lr = cfg.OPTIMIZER.TEXTURE_LR
        ...
        if self.enhance_texture:
            self.depth_model = TextureAwareDepthNet(cfg)
        else:
            self.depth_model = DepthNet(cfg)

        self.pose_model = PoseNet(cfg)
        # self.texture_model = TextureNet(cfg)
    ...
    # def texture_estimation(self, inputs):
    #     # inputs[('color', 0)],
    #     image, texture = self.texture_model(inputs["color_aug", 0], inputs['weather_depth'])

    #     return image, texture

    def shared_step(self, inputs, is_train):
        # images, texture = self.texture_estimation(inputs)
        # inputs[('color_texture', 0)] = images
        # inputs[('texture', 0)] = texture
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        if self.evaluation_quantitative_res_path:
            #depths = depth2inv(outputs[('disp', 0, 0)]).cpu().numpy()
            depths = outputs[('disp', 0, 0)].cpu().numpy()
            #print(depths)
            #depths = torch.clamp(depths, max=1.0).cpu().numpy()
            #print(depths)
            #print("123111111111111111111111111111111111111111111111111111111")
    ...
    def configure_optimizers(self):
        optimizer = optim.Adam([
            ...
            # {'params': self.texture_model.parameters(), 'lr': self.texture_lr}
        ],
