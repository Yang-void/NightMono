...
from utils.depth import viz_inv_depth, viz_disps, depth2inv
...
class Visualizer:
    ...
    def visualize_predict(self, batch, outputs):
        inv_depths = outputs[('disp', 0, 0)]#
        # inv_depths = torch.clamp(inv_depths, max=60.0)
        # inv_depths = depth2inv(inv_depths)
    ...
    def visualize_train(self, batch, outputs, logger, global_step):
        ...
        # texture_0 = {"texture_0": batch[('texture' , 0)].clone().detach().cpu()}
        # # color_aug_ref = {f"retinex_enhanced_aug_{frame_id}": batch[("color_aug", frame_id)].clone().detach().cpu() for frame_id in self.temp_context[1:]}
        # texture = {**texture_0} # **color_aug_ref}
        # self.tensorboard_add_images(texture, logger, global_step)

        # texture_teacher = {"texture_teacher": batch[('texture_teacher' , 0)].clone().detach().cpu()}
        # # color_aug_ref = {f"retinex_enhanced_aug_{frame_id}": batch[("color_aug", frame_id)].clone().detach().cpu() for frame_id in self.temp_context[1:]}
        # texture_t = {**texture_teacher} # **color_aug_ref}
        # self.tensorboard_add_images(texture_t, logger, global_step)

        # color_texture_0 = {"color_texture": batch[('color_texture', 0)] .clone().detach().cpu()}
        # # color_texture_ref = {f"color_texture_{frame_id}": batch[("color_texture", frame_id)].clone().detach().cpu() for frame_id in self.temp_context[1:]}
        # color_texture = {**color_texture_0}#, **color_aug_ref}
        # self.tensorboard_add_images(color_texture, logger, global_step)
        ...
        disparity_teacher = {"disparity_teacher": viz_disps(outputs[("disp_teacher", 0)], [8,3,768,1280])}
        self.tensorboard_add_images(disparity_teacher, logger, global_step)
        ...
        # 可视化前3个通道
        feature_vis = {}
        for i in range(min(3, texture_features.shape[1])):
            channel = texture_features[:, i:i+1]  # [B, 1, H, W]
            # 归一化到[0,1]范围
            channel_norm = (channel - channel.min()) / (channel.max() - channel.min() + 1e-6)
            feature_vis[f"texture_channel_{i}"] = channel_norm.clone().detach().cpu()

        # 计算所有通道的平均值作为整体特征图
        mean_feature = texture_features.mean(dim=1, keepdim=True)  # [B, 1, H, W]
        mean_feature_norm = (mean_feature - mean_feature.min()) / (mean_feature.max() - mean_feature.min() + 1e-6)
        feature_vis["texture_mean"] = mean_feature_norm.clone().detach().cpu()

        self.tensorboard_add_images(feature_vis, logger, global_step)

        # 可视化valid_region和depth_grad_masked
        mask_vis = {}
        if ("valid_region", 0) in outputs:
            mask_vis["valid_region"] = outputs[("valid_region", 0)].clone().detach().cpu()
        if ("depth_grad_masked", 0) in outputs:
            depth_grad = outputs[("depth_grad_masked", 0)].clone().detach().cpu()
            # 归一化到[0,1]范围用于可视化
            depth_grad_norm = (depth_grad - depth_grad.min()) / (depth_grad.max() - depth_grad.min() + 1e-6)
            mask_vis["depth_grad_masked"] = depth_grad_norm
        
        if mask_vis:
            self.tensorboard_add_images(mask_vis, logger, global_step)
