...
class RobotCarDataset:
    def __init__(self, cfg, transform):
        ...
        self.load_pso_depth = True
        self.load_sky_mask = True
    ...
    # def get_pso_depth(self, frame_idx):
    #     # 构建npy文件的路径
    #     base_filename = self.sample_data[frame_idx]['color'][0]
    #     filename = f"{os.path.join(self.sample_data[frame_idx]['scene'], 'disp', base_filename)}_depth_pred.png"
    #     filepath = os.path.join(self.dataroot, filename)
    #     depth_map = cv2.imread(filepath, cv2.IMREAD_GRAYSCALE)
    #     depth_map = depth_map.astype(np.float32) / 255.0
    #     # 从npy文件中读取数组
    #     #lidar_proj = np.load(filepath)
    ...
    def get_pso_depth(self, frame_idx):
        # 构建npy文件的路径
        base_filename = self.sample_data[frame_idx]['color'][0]
        filename = f"{os.path.join(self.sample_data[frame_idx]['scene'], 'disp1', base_filename)}_depth_pred.npy"
        filepath = os.path.join(self.dataroot, filename)
        ...
        return lidar_proj
        # 归一化
        # current_max = lidar_proj.max()
        # normalized = lidar_proj / current_max
        #return normalized 
    
    def get_sky_mask(self, frame_idx):
        # 构建sky mask文件的路径
        base_filename = self.sample_data[frame_idx]['color'][0]
        filename = f"{os.path.join(self.sample_data[frame_idx]['scene'], 'sky_mask', base_filename)}.png"  # 移除_pred后缀
        filepath = os.path.join(self.dataroot, filename)
        
        try:
            # 读取二值mask
            sky_mask = cv2.imread(filepath, cv2.IMREAD_GRAYSCALE)
            # 转换为0-1的浮点数mask
            sky_mask = (sky_mask > 0).astype(np.float32)  # 任何非0值都认为是天空区域
            return sky_mask
        except:
            # 如果读取失败，返回全0的mask
            print("读取失败了")
            return np.zeros((self.original_height, self.original_width), dtype=np.float32)
    ...
    def getitem(self, index, mode):
        ...
        if self.load_pso_depth and mode in ['train']:
            sample['pso_depth'] = torch.Tensor(self.get_pso_depth(index))

        if self.load_sky_mask and mode in ['train']:
            sample['sky_mask'] = torch.Tensor(self.get_sky_mask(index))
...
class RobotCarDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        crop = data.transforms.Crop(top=self.dataset_crop_top, left=self.dataset_crop_left,
                                    ...
                                    sample_keys=['color', 'color_aug', 'color_aug_pose', 'K', 'depth_gt', 'pso_depth', 'sky_mask'], temp_context=self.temp_context)
        ...
        to_tensor = data.transforms.ToTensor(sample_keys=['color', 'color_aug', 'color_aug_pose', 'color_original', 'depth_gt', 'pso_depth', 'sky_mask'],
                                             temp_context=self.temp_context)
