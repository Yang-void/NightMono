...
class Crop:
    def __init__(self, top, left, height, width, sample_keys, temp_context):
        ...
        self.sample_keys_contains_pso_depth = 'pso_depth' in self.sample_keys
        self.sample_keys_contains_sky_mask = 'sky_mask' in self.sample_keys
        if self.sample_keys_contains_K:
            self.sample_keys.remove('K')
        ...
        if self.sample_keys_contains_pso_depth:
            self.sample_keys.remove('pso_depth')
        if self.sample_keys_contains_sky_mask:
            self.sample_keys.remove('sky_mask')
    ...
    def __call__(self, sample):
        ...
        if self.sample_keys_contains_pso_depth and 'pso_depth' in sample:
            sample['pso_depth'] = self.crop(sample['pso_depth'])
        if self.sample_keys_contains_sky_mask and 'sky_mask' in sample:
            sample['sky_mask'] = self.crop(sample['sky_mask'])
